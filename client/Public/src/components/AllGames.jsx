        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {games?.map((game) => (
            <div key={game?.id} className="bg-white p-4 rounded-xl shadow-lg hover:shadow-xl transition-shadow group text-center">
              ...
              <button onClick={() => navigate(`/detail/${game?.id}`)} className="text-sm cursor-pointer bg-blue-100 text-blue-600 px-6 py-2 rounded-full hover:bg-blue-200 transition-colors">
                Top Up
              </button>
            </div>
          ))}
